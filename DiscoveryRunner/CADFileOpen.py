...
def runDiscoveryWithJournal():
    proc = subprocess.check_call(
        '"' + scdminstall + '"' + " /RunApollo=true /UseSubscription=true" + " /J=" + scjournalpath + " /EXITAFTERJOURNAL=true",
        shell=True)

def runDiscoveryWithScript():
    proc = subprocess.check_call(
        '"' + scdminstall + '"' + " /RunApollo=true /UseSubscription=true" + " /RunScript=" + scjournalpath+ " /ExitAfterScript=true", shell=True)
...
try:
    if scjournalpath.split(".")[1]=="scjournal":
        runDiscoveryWithJournal()
    else:
        runDiscoveryWithScript()
except:
            ...
            if scriptPass and scjournalpath.split(".")[1] != "scjournal":
                ...
                for lin in mylist:
                    if re.match(fail_pattern, lin):
                        ...
                        break
                if not LogWriting:
                    f.write(failStringPartOne + "Discovery Live session did not exit properly.Pease check LogFiles in AppData for details." + failStringPartTwo)
        f.close()
